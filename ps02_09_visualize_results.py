        ...
        def plot_station_comparison(ax, station_global_idx, title_prefix, color):
            ...
            if len(ps02c_pos) > 0:
                ...
                if fitted_params:
                    ...
                    # INVESTIGATION: PS02C linear_trend = ps00_rates (line 147 in ps02_29_phase2_optimized_implementation.py)
                    # Therefore BOTH PS00 and PS02C rates need negative sign for visualization
                    ...
                    ps00_deformation_rate = ps00_actual_slope   # PS00 follows geodetic convention
                    ps02c_deformation_rate = ps02c_actual_slope  # PS02C fitted should also follow convention
                    ax.plot(time_years, trend_original, 'b--', linewidth=1, alpha=0.6, label=f'PS00: {ps00_deformation_rate:.1f} mm/yr')
                    ax.plot(time_years, trend_fitted, 'r--', linewidth=1, alpha=0.6, label=f'PS02C: {ps02c_deformation_rate:.1f} mm/yr')
                    ...
                    ax.set_title(f'{title_prefix} Site {station_global_idx}\\nPS00: {ps00_actual_slope:.1f} mm/yr\\n(No PS02C fit)', fontsize=10)
            else:
