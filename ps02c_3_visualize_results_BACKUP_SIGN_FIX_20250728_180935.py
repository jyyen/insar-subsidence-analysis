...
class PS02CResultsVisualizer:
    ...
    def create_subsidence_validation_comparison(self, save_dir: Path) -> str:
        ...
        # Convert PS00 rates to surface deformation convention (negative = subsiding)
        ...
        def plot_station_comparison(ax, station_global_idx, title_prefix, color):
            ...
            if len(ps02c_pos) > 0:
                ...
                if fitted_params:
                    ...
                    # Convert to surface deformation convention (negative = subsiding, positive = uplifting)
                    ps00_deformation_rate = -original_rate  # Flip PS00 to geodetic convention
                    
                    # Calculate PS02C trend using actual slope from fitted time series (FIXED)
                    fitted_slope = np.polyfit(time_years, fitted_ts, 1)[0]
                    trend_fitted = fitted_slope * time_years
                    ps02c_deformation_rate = fitted_slope   # PS02C slope already matches geodetic convention
                    
                    # PS00 trend (consistent with surface deformation convention)
                    ...
                    ps00_deformation_rate = -original_rate  # Convert to surface deformation convention
                    trend_original = ps00_deformation_rate * time_years
