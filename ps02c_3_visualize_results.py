#!/usr/bin/env python3
"""
Visualize PS02C Results - Comprehensive Visualization Suite

Creates publication-quality figures from ps02c_algorithmic_fixed.py results:
1. Performance overview maps
2. Time series comparisons  
3. Statistical distributions
4. Quality metrics analysis
5. Geographic correlation patterns
...
class PS02CResultsVisualizer:
    ...
    def create_subsidence_validation_comparison(self, save_dir: Path) -> str:
        ...
        # Apply sign correction to match geodetic convention
        ps02c1_trends = -df['trend'].values  # Sign correction for geodetic convention
        ...
        # PS00 rates already follow geodetic convention (negative = subsidence, positive = uplift)
        ps00_surface_rates = original_rates[station_indices]
        
        # 1. PS00: GPS-Corrected Surface Deformation Rates (Ground Truth)
        ax1 = plt.subplot(2, 2, 1)
        scatter1 = ax1.scatter(lons, lats, 
                             c=ps00_surface_rates, s=30, alpha=0.8,
                             cmap='coolwarm', vmin=vmin, vmax=vmax,
                             edgecolors='k', linewidths=0.3)
        ax1.set_xlabel('Longitude (°)', fontsize=11)
        ax1.set_ylabel('Latitude (°)', fontsize=11)
        ax1.set_title('PS00: Surface Deformation Rates\n(Ground Truth, mm/year)', fontsize=12, pad=10)
        ax1.set_aspect('equal', adjustable='box')
        cbar1 = plt.colorbar(scatter1, ax=ax1, shrink=0.8)
        cbar1.set_label('mm/year', rotation=270, labelpad=15)
        
        # Add statistics
        ax1.text(0.02, 0.98, f'Mean: {np.nanmean(ps00_surface_rates):.1f} mm/yr\nStd: {np.nanstd(ps00_surface_rates):.1f} mm/yr',
        ...
        corr_ps00_ps02c1 = np.corrcoef(ps00_surface_rates, ps02c1_trends)[0, 1]
        ...
        corr_ps00_ps02c2 = np.corrcoef(ps00_surface_rates, ps02c2_trends)[0, 1]
        ...
        # 4. Differences: PS02C-1 vs PS00 (Ground Truth Comparison)
        ax4 = plt.subplot(2, 2, 4)
        ps02c_vs_ps00_diff = ps02c1_trends - ps00_surface_rates
        diff_range = max(abs(np.nanmin(ps02c_vs_ps00_diff)), abs(np.nanmax(ps02c_vs_ps00_diff)))
        
        # Use rainbow colormap for differences  
        scatter4 = ax4.scatter(lons, lats, 
                             c=ps02c_vs_ps00_diff, s=30, alpha=0.8,
                             cmap='coolwarm', vmin=-diff_range, vmax=diff_range,
                             edgecolors='k', linewidths=0.3)
        ax4.set_xlabel('Longitude (°)', fontsize=11)
        ax4.set_ylabel('Latitude (°)', fontsize=11)
        ax4.set_title('Residuals: PS02C-1 - PS00\n(mm/year)', fontsize=12, pad=10)
        ax4.set_aspect('equal', adjustable='box')
        cbar4 = plt.colorbar(scatter4, ax=ax4, shrink=0.8)
        cbar4.set_label('mm/year', rotation=270, labelpad=15)
        
        # Add statistics
        rmse_ps02c1 = np.sqrt(np.nanmean((ps02c1_trends - ps00_surface_rates)**2))
        rmse_ps02c2 = np.sqrt(np.nanmean((ps02c2_trends - ps00_surface_rates)**2))
        mean_residual = np.nanmean(ps02c_vs_ps00_diff)
        
        ax4.text(0.02, 0.98, f'PS02C-1 RMSE: {rmse_ps02c1:.1f} mm/yr\nPS02C-2 RMSE: {rmse_ps02c2:.1f} mm/yr\nMean Residual: {mean_residual:.2f} mm/yr\nMax |Residual|: {np.nanmax(np.abs(ps02c_vs_ps00_diff)):.1f} mm/yr',
                transform=ax4.transAxes, fontsize=9, verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
        
        # Adjust layout with more spacing to prevent text overlap
        plt.tight_layout(pad=4.0)
        plt.subplots_adjust(top=0.85, bottom=0.15, left=0.1, right=0.9, hspace=0.4, wspace=0.3)
        
        # Add overall title
        fig.suptitle('Surface Deformation Rate Comparison: PS00 vs PS02C-1 vs PS02C-2\n'
        ...
        print(f"   • PS00 GPS-Corrected: Mean={np.nanmean(ps00_surface_rates):.1f}, Std={np.nanstd(ps00_surface_rates):.1f} mm/yr")
        ...
        mean_improvement = np.nanmean(ps02c2_trends - ps02c1_trends)
    ...
    def create_extreme_sites_comparison(self, save_dir: Path) -> str:
        ...
        def plot_station_comparison(ax, station_global_idx, title_prefix, color):
            ...
            if len(ps02c_pos) > 0:
                ...
                if fitted_params:
                    ...
                    # Calculate actual slopes from time series for trend lines
                    ps00_actual_slope = np.polyfit(time_years, original_ts, 1)[0]
                    ps02c_actual_slope = np.polyfit(time_years, fitted_ts, 1)[0]
                    
                    # Trend lines MUST match their respective time series slopes
                    trend_original = ps00_actual_slope * time_years
                    trend_fitted = ps02c_actual_slope * time_years
                    
                    # For display rates, ensure geodetic convention (negative = subsidence)
                    ps00_deformation_rate = ps00_actual_slope   # PS00 already follows geodetic convention
                    ps02c_deformation_rate = ps02c_actual_slope  # PS02C fitted time series should also follow convention
                    
                    # Plot trend lines with surface deformation rates
                    ax.plot(time_years, trend_original, 'b--', linewidth=1.5, alpha=0.7, 
                           label=f'PS00: {ps00_deformation_rate:.1f} mm/yr')
                    ax.plot(time_years, trend_fitted, 'r--', linewidth=1.5, alpha=0.7,
                           label=f'PS02C: {ps02c_deformation_rate:.1f} mm/yr')
                    
                    # Calculate correlation
                    corr = np.corrcoef(original_ts, fitted_ts)[0, 1] if not np.any(np.isnan(original_ts)) else 0
                    
                    ax.set_title(f'{title_prefix} Site {station_global_idx}\nPS00: {ps00_deformation_rate:.1f} mm/yr, PS02C: {ps02c_deformation_rate:.1f} mm/yr\nCorr: {corr:.3f}', 
                                fontsize=9, pad=12)
                else:
                    # No fitted parameters available
                    ax.plot(time_years, original_ts, 'b-', linewidth=2, label='PS00 Original')
                    ps00_deformation_rate = original_rate   # PS00 already in geodetic convention
                    trend_original = ps00_deformation_rate * time_years
                    ax.plot(time_years, trend_original, 'b--', linewidth=1, alpha=0.6)
                    ax.set_title(f'{title_prefix} Site {station_global_idx}\nPS00: {ps00_deformation_rate:.1f} mm/yr\n(No PS02C fit)', fontsize=9, pad=12)
            else:
                ...
                trend_original = ps00_deformation_rate * time_years
                ax.plot(time_years, trend_original, 'b--', linewidth=1, alpha=0.6)
                ax.set_title(f'{title_prefix} Site {station_global_idx}\nPS00: {ps00_deformation_rate:.1f} mm/yr\n(Not in PS02C)', fontsize=9, pad=12)
        ...
        plt.tight_layout()
        plt.subplots_adjust(top=0.90, bottom=0.08, hspace=0.5, wspace=0.3)
