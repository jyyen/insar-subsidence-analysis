#!/usr/bin/env python3
"""
ps02_signal_decomposition.py
Signal Decomposition with EMD and Multi-Method Architecture

Purpose: Decompose InSAR time series into frequency components using EMD first,
with architecture to support FFT, VMD, and Wavelet methods later.

Key Features:
- EMD decomposition using emd-signal (CPU-based)
- Chunked processing for better reliability than multiprocessing
- Power-spectral re-categorization with peak counting verification  
- All stations processed (with subsampling by factor 500 from ps00)
- Visualization of 200-500 stations with color-coding and transparency
- All re-categorization quality strategies implemented
- Extensible architecture for future methods

GPU Acceleration Potential:
- EMD: Limited (most libraries are CPU-based)
- FFT/Spectral: Excellent (CuPy, JAX support)
- VMD: Moderate (optimization algorithms can be GPU-accelerated)
- Wavelet: Good (PyWavelets GPU backends available)
"""
